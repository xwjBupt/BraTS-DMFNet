...
class MFNet(nn.Module):  #
    ...
    def __init__(self, c=4, n=32, channels=128, groups=16, norm='bn', num_classes=4, att=False, **kwargs):
        ...
        if self.att:
            self.cor_att0 = CornerAttention(channels, **kwargs)#to verify
            self.cor_att1 = CornerAttention(channels, **kwargs)
            self.cor_att2 = CornerAttention(channels, **kwargs)
...
class CornerAttention(nn.Module):
    def __init__(self, in_channels, shape=(64, 64, 64), pooling='avg', reduction=4, wchannel=None, middle_norm='sig',
                 **kwargs):
        super(CornerAttention, self).__init__()
        ...
        self.hconv1 = nn.Sequential(
            ...
            nn.Conv3d(in_channels=in_channels, out_channels=middle_ch, kernel_size=1, stride=1, padding=0),
            nn.ReLU(inplace=True),
            nn.Dropout3d(p=0.5),
            nn.InstanceNorm3d(in_channels),
            nn.Conv3d(in_channels=middle_ch, out_channels=middle_ch, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv3d(in_channels=middle_ch, out_channels=in_channels, kernel_size=1, stride=1, padding=0),
        )
        ...
        if self.pooling == 'max':
            self.dpooling = nn.AdaptiveMaxPool3d((shape[0], 1, 1))
            self.hpooling = nn.AdaptiveMaxPool3d((1, shape[1], 1))
            self.wpooling = nn.AdaptiveMaxPool3d((1, 1, shape[1]))
        else:
            self.dpooling = nn.AdaptiveAvgPool3d((shape[0], 1, 1))
            self.hpooling = nn.AdaptiveAvgPool3d((1, shape[1], 1))
            self.wpooling = nn.AdaptiveAvgPool3d((1, 1, shape[1]))

        self.dconv2 = nn.Conv3d(in_channels, in_channels, (3, 1, 1), stride=(1, 1, 1), padding=(1, 0, 0))
        self.hconv2 = nn.Conv3d(in_channels, in_channels, (1, 3, 1), stride=(1, 1, 1), padding=(0, 1, 0))
        self.wconv2 = nn.Conv3d(in_channels, in_channels, (1, 1, 3), stride=(1, 1, 1), padding=(0, 0, 1))

        self.fuse = nn.Conv3d(in_channels, in_channels, kernel_size=1, stride=1, padding=0)
        self.sig = nn.Sigmoid()
